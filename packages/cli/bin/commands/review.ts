...
function buildReviewItems(
  ...
  return reviewableContracts.map((contract) => {
    ...
    const classification: 'breaking' | 'non-breaking' = contract.status === 'needs-review' ? 'breaking' : 'non-breaking';
    return {
      ...
      recommendedAction: classification === 'breaking' || affected.length > 0 ? 'update' : 'accept',
      availableActions: ['accept', 'update', 'reject'],
      suggestedActions: buildSuggestedActions(contract.status === 'needs-review' ? 'breaking' : 'non-breaking', affected.length),
      dependencyContext: {
