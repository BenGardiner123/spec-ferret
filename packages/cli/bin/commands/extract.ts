...
function renderContractMarkdown(id: string, type: string, shape: Record<string, unknown>, sourceFile: string, sourceSymbol: string): string {
  return `---\nferret:\n  id: ${id}\n  type: ${type}\n  shape:\n${toYaml(shape, 4)}\n---\n\n# ${id}\n\nGenerated by ferret extract from ${sourceFile} (${sourceSymbol}).\n`;
}

export const extractCommand = new Command('extract')
  .description('Deterministically scaffold .contract.md files from exported TypeScript declarations (annotation overrides supported).')
  .option('--write', 'Write generated contracts to disk (default: true)', true)
  .option('--perf-budget-ms <ms>', 'Fail (exit 1) if extract runtime exceeds this budget in milliseconds')
  .action(async (options) => {
    ...
    for (const relFile of [...files].sort()) {
      ...
      for (const contract of result.contracts) {
        ...
        const normalizedShape = normalizeSchema(contract.shape) as Record<string, unknown>;
    ...
    if (perfExceeded) {
      process.stderr.write(`ferret: performance budget exceeded for extract (${ms}ms > ${perfBudgetMs}ms).\n`);
    }
