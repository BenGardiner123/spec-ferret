...
export const lintCommand = new Command('lint')
  ...
  .option('--ci', 'Machine-readable JSON output, no ANSI codes. Exit 1 on breaking drift.')
  .option('--ci-baseline <mode>', 'CI baseline strategy: committed (default) or rebuild', 'committed')
  .option('--ci-suggestions', 'Include non-blocking import suggestions in --ci output')
  .option('--perf-budget-ms <ms>', 'Fail (exit 1) if lint runtime exceeds this budget in milliseconds')
  ...
  .action(async (options) => {
    ...
    if (perfBudgetMs === null) {
      process.stderr.write('ferret: invalid --perf-budget-ms value. Use a positive number.\n');
      process.exit(2);
      return;
    }
    ...
    const baselineMode = options.ci ? String(options.ciBaseline ?? 'committed') : undefined;
    ...
    try {
      ...
      for (const [contractId, affected] of flaggedContracts) {
        ...
        const label = isBreaking ? pc.red('BREAKING') + `  ${contractId}` : pc.yellow('NON-BREAKING') + `  ${contractId}`;
...
async function restoreCommittedBaseline(store: Awaited<ReturnType<typeof getStore>>, contextPath: string): Promise<void> {
  ...
  const allFilePaths = new Set<string>([...contractsByFilePath.keys(), ...dependencyTargetsByFilePath.keys()]);
...
async function runScan(root: string, options: { changed?: boolean; force?: boolean }): Promise<void> {
